...
function E(that, context, width, height) {
  if (that.options.data == undefined || that.options.xLabel == undefined) {
    context.draw();
    return
  }
  ...
  var animation = function() {
    if (step <= MaxStep) {
      ...
      that.options.data.forEach(function(val, idx) {
        ...
        context.arc(center.x, center.y, radius * 0.6, start, start + val.value / MaxStep * step / total * 2 * Math.PI, false);
        context.lineTo(center.x, center.y);
        context.fill();
        context.closePath();
...
function CC(that, posX, posY, width, height) {
  ...
  toolTipInfo.tooltipInfo = '';
  if (that.options && that.options.data && that.options.xLabel) {
    that.options.data.forEach(function(val) {
      if (that.options.tooltip === undefined) {
        that.options.tooltip = '{b}'
      }
      if (val.data[dataIndex] >= 0) {
        var txt = that.options.tooltip.replace(/{b}/g, val.data[dataIndex]);
        txt = txt.replace(/{a}/g, that.options.xLabel[dataIndex]);
        toolTipInfo.tooltipInfo += txt + ' '
      }
    })
  }
